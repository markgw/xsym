...
from keras.layers.core import Dense, SpatialDropout1D, Lambda, Activation, Dropout
...
class NeuralSixgramModel(object):
    ...
    def __init__(self, build_params={}):
        ...
        self.embedding_activation = self.params["embedding_activation"]
        ...
        def _apply_embeddings(inp):
            return Activation(self.embedding_activation)(squeeze_time(self.dropout_layer(self.char_embeddings(inp))))
        ...
        for i, layer_size in enumerate(self.composition2_layer_sizes):
            ...
            layer_activation = self.embedding_activation if i == len(self.composition2_layer_sizes)-1 else "tanh"
            self.composition2_layers.append(Dense(layer_size,
                                                  activation=layer_activation,
                                                  name="compose2_%d_%s" % (i, layer_activation or "linear"),
        ...
        for i, layer_size in enumerate(self.composition3_layer_sizes):
            # The last composition layer projects back into the embedding space, so is a linear layer
            layer_activation = self.embedding_activation if i == len(self.composition3_layer_sizes)-1 else "tanh"
            self.composition3_layers.append(Dense(layer_size,
                                                  activation=layer_activation,
                                                  name="compose3_%d_%s" % (i, layer_activation or "linear"),
                                                  kernel_initializer='glorot_normal',
                                                  kernel_regularizer=l2(self.l2_reg)))
    ...
    def get_embeddings(self):
        embeddings = self.char_embeddings.get_weights()[0]
        if self.embedding_activation == "linear":
            return embeddings
        elif self.embedding_activation == "softmax":
            return softmax(embeddings)
        elif self.embedding_activation == "sigmoid":
            return 1. / (1. + numpy.exp(- embeddings))
        else:
            raise NotImplementedError("embedding activation function '%s' was used, but we've not implemented the "
                                      "application of this yet" % self.embedding_activation)
